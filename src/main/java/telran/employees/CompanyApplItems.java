...
static void addEmployee(InputOutput io) {
	Menu menu = new Menu("adding Employee of reqired Type",getAddEmployeeItems());
	menu.perform(io);
...
private static Item[] getAddEmployeeItems() {
	Item [] items = {
			...
			Item.of("add SalesPerson",
			io ->addEmployeeItem(io,CompanyApplItems::getSalesPerson)),
			Item.of("add Manager",
			io ->addEmployeeItem(io,CompanyApplItems::getManager)),
			Item.ofExit()
	};
	return items;
}
